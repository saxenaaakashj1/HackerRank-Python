def main():
    ...
    if 2 <= n <= 10:
        ...
        # Read the name of the student whose average marks are to be queried
        query_name = input()

        # Check if the query_name is in the students dictionary
        # If it is, calculate the average of the student's marks and print it
        # If not, do nothing (None)
        print(f"{sum(students[query_name])/3:.2f}") if query_name in students else None

# Standard Python boilerplate to ensure the main function is called when the script is executed
